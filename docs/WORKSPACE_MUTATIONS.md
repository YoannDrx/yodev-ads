...
Le [lot 47](./audits/prod-ready-lot-47/README.md) ajoute la demande Teams temporaire au départ : le callback la consomme pour ouvrir la sélection, avec refus des demandes remplacées ou déjà utilisées. Les noms de cookies sont propres à chaque demande/session. Les formulaires et la page lient l’espace affiché et la session au contexte signé, avant tout accès fournisseur. Les erreurs de chargement ont un rendu FR/EN ; leur validation fournisseur reste distincte.

Le [lot 48](./audits/prod-ready-lot-48/README.md) applique `reports:manage` et `monitoring` aux six mutations de modèles et planifications dans leur transaction. L’analyste conserve la gestion ; les versions, liens et audits sont annulés si l’essai expire après attente. Les quotas restent fondés sur le forfait courant. Les autres mutations de publication/révocation et les garanties de bail demeurent distinctes.

Le [lot 49](./audits/prod-ready-lot-49/README.md) ajoute le garde aux trois mutations manuelles de publication/révision/révocation. Éditions, révélations et audits sont annulés lors d’une expiration d’essai après attente. La révision verrouille planification puis lien avant de lire le token ; la révocation vérifie le bail courant après acquisition du verrou. Les OTP/avis publics restent des flux distincts.

Les lots 37–49 relient les reproductions, les attentes PostgreSQL observées, les audits autoritatifs et les parcours navigateur. Ils ne terminent pas la revue des autres mutations : sessions OAuth, rapports, domaines et lifecycle conservent leurs chantiers identifiés dans le plan. Les garanties locales ne valent pas validation des intégrations déployées.
