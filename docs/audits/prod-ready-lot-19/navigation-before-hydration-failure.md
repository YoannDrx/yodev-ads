...
- Name: navigation-state-local.spec.ts >> live navigation state >> follows client navigation, keyboard and invalid accounts in en
...
> 37 |         expect(await page.evaluate(() => (window as unknown as { __navigationSentinel?: string }).__navigationSentinel)).toBe('retained')
  ...
  59 |           await expect(quick.locator(':scope > a[aria-current="page"]')).toHaveAttribute('href', '/dashboard')
  60 |           await menu.click()
