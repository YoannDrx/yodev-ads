...
if (process.env.PLAYWRIGHT_LOCAL_FIXTURE === '1') {
  test('operations counts published reports and excludes legacy or future activation', async ({ browser }) => {
    ...
    try {
      ...
      await db.query("insert into activation_milestones(workspace_id,milestone,actor_user_id,occurred_at) values($1,'first_analysis','fixture',now()),($2,'first_qualified_analysis','fixture',now()),($1,'first_qualified_analysis','fixture',now()+interval '1 day')",ids)
