...
if (process.env.PLAYWRIGHT_LOCAL_FIXTURE === '1' && process.env.PLAYWRIGHT_ANALYTICS_CONTROLS === '1') for (const [index, locale] of ['fr', 'en'].entries()) {
  test(`monitoring actor ${locale} preserves authorized controls and rejects a revoked form`, async ({ browser }) => {
    ...
    try {
      ...
      await expect(toggle.getByRole('button',{name:/^(Pause|Mettre en pause)$/})).toBeVisible()
      ...
      // A database commit precedes the action's new form render; wait before choosing the next operation.
      const incidentCard=page.locator('[data-slot="card"]').filter({has:page.getByRole('heading',{name:'Actor workflow alert',exact:true})})
      await expect(incidentCard.locator('[data-slot="badge"]').filter({hasText:locale==='fr'?/^Acquittée$/:/^Acknowledged$/})).toBeVisible()
      ...
      await expect(incidentCard.getByRole('button',{name:/^(Reopen|Rouvrir)$/})).toHaveCount(0)
