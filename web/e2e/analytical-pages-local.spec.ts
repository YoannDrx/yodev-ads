  ...
  test.describe.serial('complete analytical collections', () => {
    ...
    for (const locale of ['fr', 'en'] as const) test(`browses, searches and exports all stored rows in ${locale}`, async ({ browser }) => {
      ...
      try {
        ...
        await expect(page.getByRole('main').getByRole('alert')).toContainText(locale === 'en' ? 'collection changed' : 'collecte a changé')
        ...
        await expect(page.getByRole('main').getByRole('alert')).toContainText(locale === 'en' ? 'invalid or expired' : 'invalide ou expiré')
