...
export function AlertQualitySummary({ counts, total, english }: { counts: { useful: number; noise: number; falsePositive: number; staleReviews: number; unreviewed: number }; total: number; english: boolean }) {
  ...
  return <section aria-label={english ? 'Alert quality summary' : 'Synthèse de qualité des alertes'} className="mb-5 rounded-2xl border bg-white p-4">
...
export function AlertQualityPanel({ incident, english, canManage }: { incident: QualityIncident; english: boolean; canManage: boolean }) {
  ...
  return <section aria-label={english ? 'Alert quality review' : 'Qualité de l’alerte'} data-alert-quality-state={state} className="w-full border-t pt-4">
    ...
    {state === 'stale' && <p className="mt-2 text-sm text-amber-800">{english ? `The review covers observation ${incident.qualityOccurrence}. A new observation needs review.` : `L’avis porte sur l’observation ${incident.qualityOccurrence}. Une nouvelle observation reste à évaluer.`}</p>}
    ...
    {canManage && <form action={reviewWorkspaceAlertQuality} className="mt-3 flex flex-wrap items-end gap-2">
      ...
      <label className="grid gap-1 text-xs" htmlFor={`quality-${incident.id}`}>{english ? 'Your assessment' : 'Votre évaluation'}<select id={`quality-${incident.id}`} name="label" defaultValue={incident.qualityLabel ?? 'unreviewed'} className="h-9 min-w-44 rounded-lg border bg-white px-3 text-sm"><option value="unreviewed">{alertQualityLabel(null, english)}</option>{ALERT_QUALITY_LABELS.map((label) => <option key={label} value={label}>{alertQualityLabel(label, english)}</option>)}</select></label>
