...
export function EmptyState({
  ...
  description = 'Une fois connecté, Vigihat synchronise vos comptes clients et affiche leurs performances en direct.',
}: {
  title?: string
  description?: string
}) {
  return (
    <div className="rounded-3xl border border-dashed border-[#d9d4e8] bg-white px-6 py-16 text-center shadow-sm">
      ...
      <Button asChild className="mt-6 bg-[var(--brand-accent)] text-white">
        <Link href="/settings">
          Configurer la connexion <ArrowRight className="ml-1 size-4" />
        </Link>
      </Button>
    </div>
  )
