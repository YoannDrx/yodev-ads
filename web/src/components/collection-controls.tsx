...
export function CollectionControls({ path, query, page, locale, statuses }: { path: string; query: CollectionQuery; page: Pick<CollectionPage<unknown>, 'total' | 'nextCursor' | 'started' | 'invalidCursor'> & { items: unknown[] }; locale: 'fr' | 'en'; statuses?: readonly string[] }) {
  ...
  return <div className="my-5 space-y-3" data-collection-controls>
    <form action={path} className="flex flex-wrap items-end gap-3">
      <label className="grid gap-1 text-xs"><span>{english ? 'Search all results' : 'Rechercher dans tous les résultats'}</span><input type="search" name="q" defaultValue={criteria.q} maxLength={120} className="h-10 rounded-lg border bg-white px-3 text-sm" /></label>
      {statuses && <label className="grid gap-1 text-xs"><span>{english ? 'Status filter' : 'Filtrer par statut'}</span><select name="status" defaultValue={criteria.status ?? ''} className="h-10 rounded-lg border bg-white px-3 text-sm"><option value="">{english ? 'All statuses' : 'Tous les statuts'}</option>{statuses.map((status) => <option key={status} value={status}>{labels[status]?.[english ? 1 : 0] ?? status}</option>)}</select></label>}
      {Object.entries(criteria).filter(([key]) => !['q', 'status'].includes(key)).map(([key, value]) => <input key={key} type="hidden" name={key} value={value} />)}
      <button type="submit" className="h-10 rounded-lg bg-[#0d1722] px-4 text-sm text-white">{english ? 'Apply filters' : 'Appliquer les filtres'}</button>
      <Link href={path} className="py-2 text-sm underline">{english ? 'Reset' : 'Réinitialiser'}</Link>
    </form>
    {page.invalidCursor ? <p role="alert" className="rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm">{english ? 'This page link is invalid or expired. Return to the latest results.' : 'Ce lien de page est invalide ou expiré. Revenez aux résultats les plus récents.'} <Link href={first} className="underline">{english ? 'Latest results' : 'Résultats récents'}</Link></p> : <div className="flex flex-wrap items-center justify-between gap-3 text-sm">
