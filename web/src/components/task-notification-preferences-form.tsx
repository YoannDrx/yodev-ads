...
<form action={updateMyTaskNotificationPreferences} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            {taskPreferences?.lastError && <p role="status" className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-900">{english ? 'Your last notification could not be confirmed. Contact your workspace administrator if this persists.' : 'Votre dernière notification n’a pas pu être confirmée. Contactez l’administrateur de votre espace si cela persiste.'}</p>}
  </>
}
