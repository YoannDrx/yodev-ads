...
export function AuthSecurityControls({ locale }: { locale: string }) {
  ...
  async function addPasskey() {
    await run(async () => {
      const result = await authClient.passkey.addPasskey({
        name: english ? 'Yodev Ads passkey' : 'Passkey Yodev Ads',
  ...
  return <div className="mt-4 space-y-4">
    ...
    {message && <p role="status" className="text-sm text-[var(--y-success)]">{message}</p>}
    {error && <p role="alert" className="text-sm text-[var(--y-danger)]">{error}</p>}
    <section aria-label={english ? 'Your passkeys' : 'Vos passkeys'} className="space-y-3">
      ...
      <p className="text-sm text-muted-foreground">{english ? 'Remove a key you no longer use. To disconnect devices that are already signed in, also revoke other sessions.' : 'Supprimez une clé que vous n’utilisez plus. Pour déconnecter les appareils déjà connectés, révoquez également les autres sessions.'}</p>
      {passkeys.isPending ? <p role="status">{english ? 'Loading passkeys…' : 'Chargement des passkeys…'}</p> : passkeys.error ? <div role="alert"><p>{english ? 'Unable to load your passkeys.' : 'Impossible de charger vos passkeys.'}</p><Button type="button" variant="outline" onClick={() => void passkeys.refetch()}>{english ? 'Retry' : 'Réessayer'}</Button></div> : !passkeys.data?.length ? <p className="text-sm text-muted-foreground">{english ? 'No passkey registered.' : 'Aucune passkey enregistrée.'}</p> : <ul className="divide-y rounded-md border px-4">
        {passkeys.data.map((key) => <li key={key.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
          <span className="min-w-0 break-words text-sm">{key.name || 'Passkey'}{key.createdAt && <span className="block text-xs text-muted-foreground">{english ? 'Registered on ' : 'Enregistrée le '}{new Date(key.createdAt).toLocaleDateString(english ? 'en-GB' : 'fr-FR', { timeZone: 'UTC' })}</span>}</span>
