...
export async function GET(request: Request, context: { params: Promise<{ token: string }> }) {
  ...
  return new Response(Buffer.from(await createClientReportPdf(issued.model)), {
    headers: {
      'Content-Type': 'application/pdf',
      'Content-Disposition': `attachment; filename="rapport-${result.client.googleCustomerId}-${issued.edition.editionNumber}.pdf"`,
