...
export default async function TermsPage() {
  ...
  return (
    <LegalDocument locale={locale} title={fr ? 'Conditions générales de vente et d’utilisation' : 'Terms of sale and use'}>
      <p>{fr
        ? 'Les présentes conditions régissent l’essai et l’abonnement professionnel au service Yodev Ads, entre Yodev et l’entreprise titulaire du workspace. L’offre est exclusivement B2B. La souscription implique l’acceptation de la version affichée au Checkout.'
        : 'These terms govern professional trial and subscription access to Yodev Ads between Yodev and the business owning the workspace. The offer is B2B only. Subscription requires acceptance of the version shown at Checkout.'}</p>
      <section><h2>{fr ? '1. Service et prérequis' : '1. Service and requirements'}</h2><p>{fr
        ? 'Yodev Ads assiste l’analyse, la surveillance, le reporting et la préparation de changements Google Ads. Le client doit disposer des droits nécessaires sur chaque compte connecté et respecter les règles Google Ads. Le service ne garantit ni résultat publicitaire, ni disponibilité permanente des API tierces.'
        : 'Yodev Ads assists with Google Ads analysis, monitoring, reporting and change preparation. The customer must have the required rights to every connected account and comply with Google Ads policies. The service does not guarantee advertising outcomes or uninterrupted third-party APIs.'}</p></section>
