...
export default async function SubprocessorsPage() {
  ...
  return (
    <LegalDocument locale={locale} title={fr ? 'Liste des sous-traitants' : 'Subprocessor list'}>
      <p>{fr
        ? 'Cette liste décrit les prestataires susceptibles de traiter des données pour fournir Yodev Ads. L’activation effective dépend des intégrations configurées. Le client professionnel autorise leur recours dans les conditions du DPA.'
        : 'This list describes vendors that may process data to provide Yodev Ads. Actual use depends on configured integrations. Business customers authorise their use under the DPA.'}</p>
      ...
      {notices.length > 0 && <section><h2>{fr ? 'Changements annoncés' : 'Announced changes'}</h2><div className="space-y-4">{notices.map((notice) => <article key={notice.id} className="rounded-md border p-4"><p className="font-medium text-foreground">{notice.vendorName}</p><p className="mt-1 text-xs uppercase tracking-wide">{notice.changeType} · {new Intl.DateTimeFormat(fr ? 'fr-FR' : 'en-GB', { dateStyle: 'long', timeZone: 'Europe/Paris' }).format(notice.effectiveAt)}</p><p className="mt-3 whitespace-pre-wrap">{fr ? notice.summaryFr : notice.summaryEn}</p></article>)}</div></section>}
