...
function form(overrides: Record<string, string> = {}) {
  ...
  for (const [key, value] of Object.entries({ clientIds: JSON.stringify([clientId]), version: 'a'.repeat(64), mode: 'selection', ...overrides })) result.set(key, value)
  return result
}
describe('Account selection server action', () => {
  ...
  it('uses authenticated scope and preserves the requested priority-only mode', async () => {
    await expect(updateManagedAccounts(form({ workspaceId: 'foreign', mode: 'priorities' }))).rejects.toThrow('redirect:/accounts?selection=saved')
  ...
  it('never writes when the role or lifecycle denies permission', async () => {
    ...
    await expect(updateManagedAccounts(form())).rejects.toThrow('denied')
