...
export default async function SignUpPage({ searchParams }: { searchParams: Promise<{ returnTo?: string; error?: string }> }) {
  ...
  return <AuthPanel mode="sign-up" locale={await getLocale()} googleEnabled={googleEnabled} returnTo={query.returnTo} linkError={Boolean(query.error)} publicRegistration={featureEnabled('publicBeta')} />
}
