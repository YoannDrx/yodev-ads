...
export default async function StatusPage({ searchParams }: { searchParams: Promise<{ cursor?: string; status?: string }> }) {
  ...
  return <main className="min-h-screen bg-card px-4 py-10 text-foreground sm:px-7">
    <div className="mx-auto max-w-4xl">
      <header className="flex items-center justify-between"><Link href="/" className="flex items-center gap-3 font-semibold"><span className="grid size-10 place-items-center rounded-md bg-primary"><Radar className="size-5" /></span>Yodev Ads</Link><Link href="/sign-in" className="text-sm text-muted-foreground hover:text-foreground">{english ? 'Sign in' : 'Connexion'}</Link></header>
      <section className={`mt-10 rounded-md p-7 text-foreground ${declaredIncident ? 'bg-amber-700' : 'bg-slate-700'}`}>
        <div className="flex items-start gap-4">
          {declaredIncident ? <AlertTriangle className="mt-1 size-7 shrink-0" /> : <CircleHelp className="mt-1 size-7 shrink-0" />}
          <div>
            <p className="text-xs font-semibold uppercase tracking-[.16em] text-muted-foreground">{english ? 'Service status' : 'État du service'}</p>
      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-2">{PLATFORM_COMPONENTS.map((component) => {
        ...
        return <Card key={component} className="shadow-none"><CardContent className="flex items-center justify-between gap-3 p-5"><div className="flex items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-md bg-muted"><Icon className="size-5" /></span><div><p className="font-medium">{componentCopy[component][locale]}</p><p className="mt-1 text-xs text-muted-foreground">{status ? statusCopy[status][locale] : (english ? 'Unknown' : 'Inconnu')}</p></div></div><span className={`size-2.5 shrink-0 rounded-md ${status && status !== 'unknown' ? 'bg-amber-500' : 'bg-slate-400'}`} /></CardContent></Card>
      })}</section>
      <section className="mt-10">
        ...
        {snapshot && !snapshot.page.invalidCursor && snapshot.page.total === 0 && <p className="rounded-md border border-dashed bg-card p-8 text-center text-sm text-muted-foreground">{english ? 'No declared public incident matches this filter and period.' : 'Aucun incident public déclaré ne correspond à ce filtre et à cette période.'}</p>}
      </section>
    </div>
  </main>
}
