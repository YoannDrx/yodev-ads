...
const state = { state: 'returned-state', workspaceId, userId: 'actor-1', expiresAt: Date.now() + 600_000, payload: { codeVerifier: 'pkce-fixture' } }
const request = () => GET(new Request('https://ads.example.test/api/connectors/teams/callback?code=fixture-code&state=returned-state'))
describe('Teams callback persistence deadline', () => {
  beforeEach(() => {
    ...
    mocks.create.mockResolvedValue({ id: 'fixture-session', expiresAt: new Date(state.expiresAt) })
  ...
  it('passes the signed authorization deadline to persistence after exchange', async () => {
    expect((await request()).headers.get('location')).toBe('https://ads.example.test/settings/teams')
    expect(mocks.create).toHaveBeenCalledWith({ workspaceId, actorUserId: 'actor-1', refreshToken: 'fixture-token', scopes: ['ChannelMessage.Send'], authorizationExpiresAt: new Date(state.expiresAt) })
    expect(mocks.seal).toHaveBeenCalledWith(expect.objectContaining({ payload: { sessionId: 'fixture-session' } }))
  })
  it('does not issue a selection session cookie when persistence refuses stale authorization', async () => {
    ...
    expect(mocks.set.mock.calls.some(([name]) => name === 'yodev_ads_teams_session')).toBe(false)
  })
