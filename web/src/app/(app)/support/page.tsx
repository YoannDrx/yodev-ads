  ...
  return (
    <>
      ...
      <section className="mb-6 grid gap-4 sm:grid-cols-3">
        ...
        <Card className="shadow-none"><CardContent className="flex items-center gap-3 p-5"><ShieldCheck className="size-5 text-[var(--y-success)]" /><p className="text-sm text-muted-foreground">{english ? 'Messages are visible only to your workspace and Yodev support.' : 'Les messages ne sont visibles que par votre workspace et le support Yodev.'}</p></CardContent></Card>
      </section>

      {canContact && <Card className="mb-6 border-border shadow-none">
        ...
        <CardContent><form action={createSupportTicket} className="grid gap-3 md:grid-cols-2">
          ...
          <select name="category" className="h-10 rounded-lg border bg-card px-3 text-sm" aria-label={english ? 'Category' : 'Catégorie'}>
            {Object.entries(categoryLabels).map(([value, label]) => <option key={value} value={value}>{english ? label.en : label.fr}</option>)}
          </select>
          <select name="priority" className="h-10 rounded-lg border bg-card px-3 text-sm" aria-label={english ? 'Priority' : 'Priorité'} defaultValue="normal">
      ...
      <div className="space-y-4">
        {tickets.map(({ ticket, messages, hasMoreComments }) => <Card key={ticket.id} className="[content-visibility:auto] border-border shadow-none"><CardContent className="p-5">
          ...
          <DiscussionLink kind="support" id={ticket.id} more={hasMoreComments} locale={english ? 'en' : 'fr'} /><div className="mt-5 space-y-3 border-t pt-4">{messages.map((supportMessage) => <div key={supportMessage.id} className={`rounded-md px-4 py-3 ${supportMessage.authorKind === 'support' ? 'y-status-success' : 'bg-muted'}`}><p className="whitespace-pre-wrap text-sm leading-6">{supportMessage.body}</p><p className="mt-2 text-[11px] text-muted-foreground">{supportMessage.authorKind === 'support' ? 'Yodev Support' : supportMessage.authorUserId === session.userId ? (english ? 'You' : 'Vous') : (english ? 'Workspace member' : 'Membre du workspace')} · {supportMessage.createdAt.toLocaleString(english ? 'en-GB' : 'fr-FR')}</p></div>)}</div>
