...
export default async function PortfolioPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <>
    ...
    {invalidFilters && <p role="alert" className="mb-4 rounded-lg border border-amber-300 y-status-warning p-3 text-sm">{english ? 'Unsupported filters were reset. Choose the filters below.' : 'Les filtres non reconnus ont été réinitialisés. Choisissez les filtres ci-dessous.'}</p>}
    {typeof raw.view_notice === 'string' && ['saved','deleted','conflict','quota','unavailable'].includes(raw.view_notice) && <p role={['saved','deleted'].includes(raw.view_notice) ? 'status' : 'alert'} className="mb-4 rounded-lg border bg-card p-3 text-sm">{{
    ...
    <details className="mb-5 rounded-md border bg-card p-4" data-portfolio-saved-views>
      ...
      {canSaveView && <form action={storePortfolioView} className="mt-4 flex flex-wrap items-end gap-3">
        ...
        <button disabled={savedViews.length >= MAX_PORTFOLIO_VIEWS} className="h-10 rounded-lg bg-card px-4 text-sm text-foreground disabled:opacity-50">{english ? 'Save current filters' : 'Enregistrer ces filtres'}</button>
      </form>}
    ...
    <form action="/portfolio" className="mb-6 flex flex-wrap items-end gap-3 rounded-md border bg-card p-4">
      ...
      <label className="grid gap-1 text-xs">{english ? 'Priority filter' : 'Filtrer les priorités'}<select name="attention" defaultValue={query.attention} className="h-10 rounded-lg border bg-card px-3 text-sm">{Object.entries(attentionLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
      ...
      <label className="grid gap-1 text-xs">{english ? 'Task assignee' : 'Responsable des tâches'}<select name="assignee" defaultValue={query.assignee} className="h-10 max-w-64 rounded-lg border bg-card px-3 text-sm">
      ...
      <button className="h-10 rounded-lg bg-card px-4 text-sm text-foreground">{english ? 'Apply filters' : 'Appliquer les filtres'}</button>
    ...
    {page.invalidCursor ? <p role="alert" className="mb-5 rounded-lg border border-amber-300 y-status-warning p-3 text-sm">{english ? 'This page link is invalid or expired.' : 'Ce lien de page est invalide ou expiré.'} <Link href={firstPage} className="underline">{english ? 'Latest results' : 'Résultats récents'}</Link></p> : <>
      ...
      <section className="mt-7">
        ...
        <div className="mt-4 overflow-x-auto rounded-md border bg-card" tabIndex={0} role="region" aria-label={english ? 'Account metrics table' : 'Tableau des mesures par compte'}>
          <table className="w-full min-w-[1100px] text-left text-sm"><thead className="border-b bg-muted text-xs text-muted-foreground"><tr>{[english ? 'Account and coverage' : 'Compte et couverture', english ? '30-day spend' : 'Investissement 30 j', 'Conversions / CPA', 'ROAS', english ? 'Monthly pacing' : 'Pacing du mois', english ? 'Alerts' : 'Alertes', english ? 'Tasks' : 'Tâches', english ? 'Decisions' : 'Décisions'].map((label) => <th key={label} scope="col" className="px-4 py-3">{label}</th>)}</tr></thead><tbody>
