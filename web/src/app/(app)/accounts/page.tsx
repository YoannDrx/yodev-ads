...
export default async function AccountsPage({ searchParams }: { searchParams: Promise<{ notice?: string; error?: string; selection?: string }> }) {
  ...
  const error = query.selection === 'workspace_changed' ? (english ? 'The active workspace changed. Review this workspace’s accounts before saving.' : 'L’espace actif a changé. Vérifiez les comptes de cet espace avant d’enregistrer.') : query.selection === 'conflict' ? (english ? 'Another change was saved meanwhile. Your selection was not saved. Review the refreshed inventory before trying again.' : 'Une autre modification a été enregistrée entre-temps. Votre sélection n’a pas été enregistrée. Vérifiez l’inventaire actualisé avant de recommencer.') : query.selection === 'unavailable' ? (english ? 'Selection could not be saved. Check your current permissions, plan quota and available accounts.' : 'La sélection n’a pas pu être enregistrée. Vérifiez vos droits actuels, le quota du forfait et les comptes disponibles.') : query.error
  return <>
    ...
    <AccountSelectionEditor key={`${workspace.id}:${selection.version}`} workspaceId={workspace.id} {...selection} canManage={canManage} locale={english ? 'en' : 'fr'} />
  </>
}
