    ...
    <>
      ...
      <Card className="mb-6 border-border ">
        <CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex items-start gap-3">
            <span className="grid size-10 shrink-0 place-items-center rounded-md bg-indigo-50 text-indigo-700"><Globe2 className="size-5" /></span>
          ...
          <form action={updateWorkspaceLocale} className="flex gap-2">
            <select name="locale" defaultValue={locale} disabled={!isAdmin} aria-label={english ? 'Workspace language' : 'Langue de l’espace'} className="h-10 rounded-lg border bg-card px-3 text-sm"><option value="fr">Français</option><option value="en">English</option></select>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card className="border-border  xl:col-span-2">
          <CardHeader>
            <div className="flex items-center gap-3">
              <span className="grid size-10 place-items-center rounded-md bg-indigo-50 text-indigo-700"><UsersRound className="size-5" /></span>
          ...
          <CardContent>
            {!isAdmin ? (
              ...
              <p className="rounded-md y-status-warning p-4 text-sm text-[var(--y-warning)]">{english ? 'Better Auth member management is temporarily unavailable.' : 'La gestion des membres Better Auth est temporairement indisponible.'}</p>
            ) : (
              <div className="space-y-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  {!canCollaborate && <p className="text-xs text-[var(--y-warning)]">{english ? 'Invitations are available from Studio.' : 'Les invitations sont disponibles à partir de Studio.'}</p>}
                </div>
                {canCollaborate && (
                  <form action={inviteWorkspaceMember} className="grid gap-3 md:grid-cols-[1fr_180px_auto]">
                    ...
                    <select name="role" aria-label={english ? 'Role' : 'Rôle'} defaultValue="client" className="h-10 rounded-lg border bg-card px-3 text-sm"><option value="admin">Admin</option><option value="strategist">Strategist</option><option value="analyst">Analyst</option><option value="client">Client</option></select>
                    <Button type="submit"><Plus className="mr-2 size-4" />{english ? 'Invite' : 'Inviter'}</Button>
                  </form>
                )}
                <div className="space-y-2">
                  {memberRoster.members.map((member) => (
                    <div key={member.id} className="flex flex-col gap-3 rounded-md bg-card px-4 py-3 md:flex-row md:items-center md:justify-between">
                ...
                {session.userId === workspace.ownerUserId && memberRoster.members.some((member) => member.userId !== workspace.ownerUserId) && (
                  <details className="rounded-md border border-amber-200 y-status-warning p-4">
                    ...
                    <form action={transferWorkspaceOwnership} className="mt-3 grid gap-3 md:grid-cols-[1fr_1fr_auto]">
                      ...
                      <Button type="submit" variant="outline" className="border-amber-400 text-[var(--y-warning)]">{english ? 'Transfer' : 'Transférer'}</Button>
                    </form>
                  </details>
                )}
              </div>
        ...
        <Card className="border-border ">
          <CardHeader>
            <div className="flex items-center gap-3">
              <span className="grid size-10 place-items-center rounded-md bg-violet-50 text-violet-700">
          ...
          <CardContent>
            {connection ? (
                ...
                {isAdmin && (
                  <div className="flex flex-wrap gap-2">
                    <form action={syncGoogleAdsAccounts}>
                      <Button type="submit" className="bg-[var(--brand-accent)] text-foreground">
              ...
              <div>
                <div className="mb-5 flex items-start gap-3 rounded-md bg-card p-4">
                ...
                {isAdmin ? (
                  <form action="/api/google-ads/connect" method="get" className="space-y-3">
                    ...
                    <Button
                      ...
                      className="w-full bg-[var(--brand-accent)] text-foreground"
                    >
        ...
        <Card className="border-border ">
          <CardHeader>
            <div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-md y-status-success text-[var(--y-success)]"><Workflow className="size-5" /></span><div><CardTitle>{english ? 'Approval policy' : 'Politique d’approbation'}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{english ? 'Separate proposal and execution according to your plan.' : 'Séparez proposition et exécution selon votre offre.'}</p></div></div>
          </CardHeader>
          <CardContent>
            <form action={updateApprovalPolicy} className="space-y-4">
              <div className="space-y-2"><Label htmlFor="requiredApprovals">{english ? 'Required approvals' : 'Approbations requises'}</Label><select id="requiredApprovals" name="requiredApprovals" defaultValue={workspace.requiredApprovals} disabled={!isAdmin} className="h-10 w-full rounded-lg border bg-card px-3 text-sm"><option value="1">1</option><option value="2" disabled={!entitlements.capabilities.has('approvals.dual')}>2</option></select></div>
          ...
          <CardContent>
            ...
            {isAdmin && canUseBranding && blobReady && (
              <div className="mt-5 border-t pt-5">
                ...
                {isControlledBrandLogoUrl(workspace.logoUrl) && <div className="mt-3 flex items-center justify-between rounded-md bg-card p-3"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-lg bg-card text-sm font-bold text-[var(--brand-accent)]">{workspace.brandName.slice(0, 2).toUpperCase()}</span><span className="text-sm">{english ? 'Controlled logo active' : 'Logo contrôlé actif'}</span></div><form action={removeWorkspaceLogo}><Button type="submit" size="sm" variant="ghost">{english ? 'Remove' : 'Retirer'}</Button></form></div>}
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-border ">
          <CardHeader>
            <div className="flex items-center gap-3">
              <span className="grid size-10 place-items-center rounded-md y-status-warning text-[var(--y-warning)]"><Gauge className="size-5" /></span>
          ...
          <CardContent>
            {isAdmin && entitlements.capabilities.has('notifications.webhook') && (
              <div className="mb-5 grid gap-3 md:grid-cols-2">
                {slackReady && <div className="flex flex-col justify-between gap-3 rounded-md border border-border bg-card p-4">
                ...
                {teamsReady && <div className="flex flex-col justify-between gap-3 rounded-md border border-border bg-card p-4">
            ...
            {isAdmin && (
              <form action={createNotificationChannel} className="grid gap-3 sm:grid-cols-2">
                ...
                <select name="kind" aria-label={english ? 'Channel type' : 'Type de canal'} className="h-10 rounded-lg border bg-card px-3 text-sm"><option value="email">Email</option>{slackReady && <option value="slack">Slack</option>}{teamsReady && <option value="teams">Teams</option>}<option value="webhook">Webhook</option></select>
                ...
                <select name="minimumSeverity" aria-label={english ? 'Minimum severity' : 'Sévérité minimale'} className="h-10 rounded-lg border bg-card px-3 text-sm"><option value="warning">{english ? 'Warnings and critical alerts' : 'Alertes et critiques'}</option><option value="critical">{english ? 'Critical only' : 'Critiques uniquement'}</option></select>
                <Button type="submit"><Plus className="mr-2 size-4" />{english ? 'Add' : 'Ajouter'}</Button>
              </form>
            )}
            <div className="mt-5 space-y-2">
              {channels.map((channel) => (
                <div key={channel.id} className="flex items-center justify-between rounded-md bg-card px-4 py-3">
              ...
              {channels.length === 0 && <p className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">{english ? 'No active channel. Incidents remain available in the cockpit.' : 'Aucun canal actif. Les incidents restent disponibles dans le cockpit.'}</p>}
            </div>
          </CardContent>
        </Card>}

        {notificationsEnabled && <Card className="border-border  xl:col-span-2">
          <CardHeader>
            <div className="flex items-center gap-3">
              <span className="grid size-10 place-items-center rounded-md bg-teal-50 text-teal-700"><UserRound className="size-5" /></span>
        ...
        <Card className="border-border  xl:col-span-2">
          <CardContent className="flex items-start gap-4 p-5">
            <span className="grid size-10 shrink-0 place-items-center rounded-md y-status-success text-[var(--y-success)]">
            ...
            <div>
              ...
              <Link href="/account" className="mt-4 inline-block text-sm font-medium text-muted-foreground underline">{english ? 'Manage my account security' : 'Gérer la sécurité de mon compte'}</Link>
            </div>
          </CardContent>
        </Card>

        {(canUseCustomDomain || domains.length > 0) && (
          <Card role="region" aria-labelledby="custom-domain-title" className="border-border  xl:col-span-2">
            <CardContent className="p-6">
              ...
              {!customDomainsEnabled && <p className="mt-4 text-sm text-[var(--y-warning)]">{english ? 'Custom domain operations are temporarily unavailable.' : 'Les opérations sur les domaines personnalisés sont temporairement indisponibles.'}</p>}
              {customDomainsEnabled && !canUseCustomDomain && <p className="mt-4 text-sm text-[var(--y-warning)]">{english ? 'Your current plan does not include custom domains. You can still remove the existing domain.' : 'Votre forfait actuel ne comprend pas les domaines personnalisés. Vous pouvez toujours retirer le domaine existant.'}</p>}
              ...
              <div className="mt-5 space-y-3">
                {domains.map((domain) => (
                  <div key={domain.id} className="rounded-md bg-card p-4">
                    ...
                    {domain.vercelConfiguration && <details className="mt-3 text-xs"><summary className="cursor-pointer text-muted-foreground">{english ? 'DNS configuration returned by Vercel' : 'Configuration DNS retournée par Vercel'}</summary><pre className="mt-2 overflow-x-auto whitespace-pre-wrap rounded-lg bg-card p-3">{JSON.stringify(domain.vercelConfiguration, null, 2)}</pre></details>}
                    {domain.lastError && <p className="mt-3 text-xs text-[var(--y-danger)]">{localizeFlashMessage(domainActionError(new Error(domain.lastError)), locale)}</p>}
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        {isAdmin && (
          <Card className="border-border  xl:col-span-2">
            <CardContent className="p-6">
              <div className="flex items-start gap-4"><span className="grid size-10 shrink-0 place-items-center rounded-md y-status-danger text-[var(--y-danger)]"><Workflow className="size-5" /></span><div><h2 className="font-semibold">{english ? 'Operational dead letter' : 'Dead-letter opérationnelle'}</h2><p className="mt-1 text-sm text-muted-foreground">{english ? 'Jobs that exhausted their attempts. Retrying is audited and adds five attempts without reusing an attempt number.' : 'Jobs arrivés au terme de leurs tentatives. La relance est auditée et ajoute cinq tentatives sans réutiliser un numéro d’essai.'}</p></div></div>
              {deadLetters.length === 0 ? <p className="mt-5 rounded-md border border-dashed p-4 text-sm text-muted-foreground">{english ? 'No dead-letter job.' : 'Aucun job en dead-letter.'}</p> : <div className="mt-5 space-y-2">{deadLetters.map((job) => <div key={job.id} className="flex flex-col justify-between gap-3 rounded-md bg-card px-4 py-3 sm:flex-row sm:items-center"><div><p className="text-sm font-medium">{job.type}</p><p className="mt-1 text-xs text-muted-foreground">{job.lastError ?? (english ? 'Undocumented error' : 'Erreur non documentée')} · {job.attemptCount}/{job.maximumAttempts} {english ? 'attempts' : 'tentatives'}</p></div><form action={retryDeadLetterJob}><input type="hidden" name="jobId" value={job.id} /><Button type="submit" size="sm" variant="outline"><RefreshCw className="mr-2 size-4" />{english ? 'Retry' : 'Relancer'}</Button></form></div>)}</div>}
            </CardContent>
          </Card>
        )}

        {canUsePrivateApi && <Card className="border-border  xl:col-span-2">
          <CardContent className="p-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-muted-foreground">
                  {english ? 'Connect Codex, Claude Code or your internal tools to the Yodev Ads portfolio with a revocable key.' : 'Branchez Codex, Claude Code ou vos outils internes sur le portefeuille Yodev Ads via une clé révocable.'}
                </p>
              </div>
            </div>
            ...
            <div className="mt-5 space-y-2">
              {keys.map((key) => (
                <div key={key.id} className="flex items-center justify-between rounded-md bg-card px-4 py-3">
                  <div>
