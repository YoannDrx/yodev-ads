...
export default async function DiscussionPage({ params, searchParams }: { params: Promise<{ kind: string; id: string }>; searchParams: Promise<CollectionQuery> }) {
  ...
  return <>
    ...
    <div className="space-y-3">{page.items.map((comment) => <article key={comment.id} className="rounded-md border bg-card p-5"><p className="break-words whitespace-pre-wrap text-sm leading-7">{comment.body}</p><p className="mt-3 text-xs text-muted-foreground">{comment.authorKind === 'support' ? 'Yodev Support' : comment.authorUserId === session.userId ? english ? 'You' : 'Vous' : english ? 'Workspace member' : 'Membre du workspace'} · {comment.createdAt.toLocaleString(locale)}</p></article>)}</div>
    {!page.invalidCursor && page.items.length === 0 && <p className="rounded-md border border-dashed p-8 text-center text-sm">{english ? 'No matching message.' : 'Aucun message correspondant.'}</p>}
  </>
}
