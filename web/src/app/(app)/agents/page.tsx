    ...
    <>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
        {agentTemplates.map((template) => (
          <Card key={template.kind} className="group overflow-hidden border-border shadow-none">
            ...
            <CardContent className="p-5">
              ...
              {canManage && (
                <form action={createMonitoringAgent} className="mt-5 space-y-3 border-t pt-4">
                  ...
                  <select
                    name="clientId"
                    className="h-9 w-full rounded-lg border bg-card px-2 text-xs"
                  ...
                  <select name="reminderIntervalHours" defaultValue="" className="h-9 w-full rounded-lg border bg-card px-2 text-xs" aria-label={english ? 'Unresolved alert reminders' : 'Rappel des alertes non traitées'}>
      ...
      <div className="mt-9 flex items-center justify-between">
        ...
        <span className="flex items-center gap-2 text-xs text-[var(--y-success)]">
          <ShieldCheck className="size-4" /> {english ? 'Approval required' : 'Approbation obligatoire'}
        </span>
      </div>
      <div className="mt-4 space-y-3">
        {agents.map(({ agent, client }) => (
          <Card key={agent.id} className="border-border shadow-none">
            <CardContent className="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
              <div className="flex min-w-0 gap-4">
                <span className="grid size-11 shrink-0 place-items-center rounded-md bg-card text-primary">
