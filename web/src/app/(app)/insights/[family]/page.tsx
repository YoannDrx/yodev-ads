...
export default async function AnalyticalCollectionPage({ params, searchParams }: { params: Promise<{ family: string }>; searchParams: Promise<{ client?: string; q?: string; cursor?: string }> }) {
  ...
  return <>
    ...
    <details className="my-4"><summary className="cursor-pointer text-sm">{english ? 'Other collections' : 'Autres collectes'}</summary><nav aria-label={english ? 'Analytical collections' : 'Collectes analytiques'} className="mt-3 flex flex-wrap gap-3">{analyticalFamilies.map((key) => <Link prefetch={false} key={key} aria-current={key === family ? 'page' : undefined} className="rounded-lg border bg-white px-3 py-2 text-sm underline" href={`/insights/${key}?client=${client.id}`}>{ANALYTICAL_FAMILIES[key][locale]}</Link>)}</nav></details>
    {snapshot && <section className="my-5 space-y-2 rounded-xl border bg-white p-4" aria-label={english ? 'Collection coverage' : 'Couverture de la collecte'}>
    ...
    <form action={path} className="my-5 flex flex-wrap items-end gap-3">
      ...
      <label className="grid min-w-0 gap-1 text-xs"><span>{english ? 'Search all stored results' : 'Rechercher dans tous les résultats enregistrés'}</span><input type="search" name="q" maxLength={120} defaultValue={page.query} className="h-10 min-w-0 rounded-lg border bg-white px-3 text-sm" /></label>
      <button type="submit" className="h-10 rounded-lg bg-[#0d1722] px-4 text-sm text-white">{english ? 'Search' : 'Rechercher'}</button>
      <Link prefetch={false} href={`${path}?client=${client.id}`} className="py-2 text-sm underline">{english ? 'Reset' : 'Réinitialiser'}</Link>
    </form>
    {page.invalidCursor || page.changed ? <p role="alert" className="my-4 rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm">{page.changed ? (english ? 'The collection changed during browsing. Restart with the latest collection.' : 'La collecte a changé pendant votre consultation. Reprenez avec la dernière collecte.') : (english ? 'This page link is invalid or expired.' : 'Ce lien de page est invalide ou expiré.')} <Link prefetch={false} href={first} className="underline">{english ? 'First page' : 'Première page'}</Link></p>
      : !snapshot ? <p className="my-5 rounded-xl border border-dashed p-6 text-sm">{english ? 'No usable collection is available for this section. This does not mean there is no activity.' : 'Aucune collecte exploitable n’est disponible pour cette section. Cela ne signifie pas une absence d’activité.'}</p>
