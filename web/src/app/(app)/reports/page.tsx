    ...
    <>
      ...
      {canManage && (
        <div className="grid gap-6 xl:grid-cols-2">
          <Card className="border-border shadow-none">
            <CardHeader><CardTitle className="flex items-center gap-2"><Plus className="size-5 text-muted-foreground" />{english ? 'Create one-off report' : 'Créer un rapport ponctuel'}</CardTitle></CardHeader>
            <CardContent>
              <form key={`${workspace.id}:createShareLink`} action={createShareLink} className="space-y-4"><input type="hidden" name="workspaceId" value={workspace.id} />
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label={english ? 'Client account' : 'Compte client'} htmlFor="report-client"><select id="report-client" name="clientId" className="h-10 w-full rounded-lg border bg-card px-3 text-sm" required>{advertiserClients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select></Field>
                  <Field label={english ? 'Language' : 'Langue'} htmlFor="report-locale"><select id="report-locale" name="locale" defaultValue={locale} className="h-10 w-full rounded-lg border bg-card px-3 text-sm"><option value="fr">Français</option><option value="en">English</option></select></Field>
                  <ReportPeriodFields id="report-period" locale={locale} />
                </div>
                <Field label={english ? 'Report type' : 'Type de rapport'} htmlFor="report-mode"><select id="report-mode" name="mode" defaultValue="fixed" className="h-10 w-full rounded-lg border bg-card px-3 text-sm"><option value="fixed">{english ? 'Immutable dated report' : 'Bilan daté et figé'}</option><option value="dynamic">{english ? 'Dynamic link to current stored data' : 'Lien dynamique vers les données enregistrées actuelles'}</option></select></Field>
          ...
          <Card className="border-border shadow-none">
            <CardHeader><CardTitle className="flex items-center gap-2"><FileText className="size-5 text-muted-foreground" />{english ? 'Create editorial template' : 'Créer un modèle éditorial'}</CardTitle></CardHeader>
            <CardContent>
              <form key={`${workspace.id}:createReportTemplate`} action={createReportTemplate} className="space-y-4"><input type="hidden" name="workspaceId" value={workspace.id} />
                <div className="grid gap-4 sm:grid-cols-3">
                  ...
                  <Field label={english ? 'Language' : 'Langue'} htmlFor="template-locale"><select id="template-locale" name="locale" defaultValue={locale} className="h-10 w-full rounded-lg border bg-card px-3 text-sm"><option value="fr">Français</option><option value="en">English</option></select></Field>
        ...
        <section className="mt-6">
          ...
          <div className="mt-3 grid gap-3 lg:grid-cols-2">
            {automation.templates.map((template) => (
              <Card key={template.id} className="border-border shadow-none">
                <CardContent className="p-5">
                  <form key={`${workspace.id}:updateReportTemplate`} action={updateReportTemplate} className="space-y-3"><input type="hidden" name="workspaceId" value={workspace.id} />
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      ...
                      <select name="locale" defaultValue={template.locale} aria-label={english ? 'Template language' : 'Langue du modèle'} className="h-10 rounded-lg border bg-card px-3 text-sm"><option value="fr">Français</option><option value="en">English</option></select>
      ...
      {canManage && schedulesEnabled && (
        <Card className="mt-6 border-border shadow-none">
          <CardHeader><CardTitle className="flex items-center gap-2"><CalendarClock className="size-5 text-muted-foreground" />{english ? 'Schedule delivery' : 'Programmer un envoi'}</CardTitle></CardHeader>
          <CardContent>
            <form key={`${workspace.id}:createReportSchedule`} action={createReportSchedule} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><input type="hidden" name="workspaceId" value={workspace.id} />
              ...
              <Field label={english ? 'Account' : 'Compte'} htmlFor="schedule-client"><select id="schedule-client" name="clientId" className="h-10 w-full rounded-lg border bg-card px-3 text-sm" required>{advertiserClients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select></Field>
              <Field label={english ? 'Template' : 'Modèle'} htmlFor="schedule-template"><select id="schedule-template" name="templateId" className="h-10 w-full rounded-lg border bg-card px-3 text-sm"><option value="">{english ? 'Standard 30-day report' : 'Rapport standard 30 jours'}</option>{automation.templates.filter((template) => { try { storedReportPeriod(template); return true } catch { return false } }).map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></Field>
              <Field label={english ? 'Cadence' : 'Cadence'} htmlFor="schedule-cadence"><select id="schedule-cadence" name="cadence" className="h-10 w-full rounded-lg border bg-card px-3 text-sm"><option value="monthly">{english ? 'Monthly' : 'Mensuelle'}</option><option value="weekly">{english ? 'Weekly' : 'Hebdomadaire'}</option></select></Field>
      ...
      {!schedulesEnabled && <p className="mt-6 rounded-md y-status-warning p-4 text-sm text-[var(--y-warning)]">{english ? 'Scheduled delivery is temporarily paused. Dated reports and their downloads remain available.' : 'Les envois programmés sont temporairement suspendus. Les bilans datés et leurs téléchargements restent disponibles.'}</p>}
      <section className="mt-8">
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-2">
          {automation.schedules.map(({ schedule, client, template }) => (
            <Card key={schedule.id} className="border-border shadow-none">
              <CardContent className="flex h-full flex-col justify-between gap-4 p-5">
                <div>
                  ...
                  {schedule.lastError && <p className="mt-2 rounded-lg y-status-danger px-3 py-2 text-xs text-[var(--y-danger)]">{schedule.lastError}</p>}
                </div>
          ...
          {automation.schedules.length === 0 && <div className="rounded-md border border-dashed bg-card p-10 text-center text-muted-foreground">{english ? 'No scheduled delivery.' : 'Aucun envoi planifié.'}</div>}
        </div>
      </section>
