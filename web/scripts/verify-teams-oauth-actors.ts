...
import { createTeamsOAuthSession, accessTeamsOAuthSession, completeTeamsOAuthSession, beginTeamsOAuthSession } from '../src/lib/notification-oauth-management'
...
function create() { return createTeamsOAuthSession({ ...context, authorizationId: sessionId, refreshToken: newToken, scopes: ['ChannelMessage.Send'], authorizationExpiresAt: new Date(Date.now() + 60_000) }) }
...
async function restore(pending = false) {
  ...
  await db.query("insert into notification_oauth_sessions(id,workspace_id,user_id,provider,encrypted_refresh_token,expires_at) values($1,$2,$3,'teams',$4,clock_timestamp()+interval '1 minute')", [sessionId, workspaceId, actor, encryptSecret(pending ? 'yodev:teams:authorization-pending' : oldToken)])
}
...
async function main() {
  ...
  try {
    ...
    for (const name of Object.keys(operations) as (keyof typeof operations)[]) {
      for (const role of ['analyst', 'strategist', 'client']) { await restore(name === 'create'); await db.query('update auth_members set role=$1 where id=$2', [role, actor]); await denied(name) }
      await db.query('delete from auth_members where id=$1', [actor]); await denied(name)
      for (const state of ['grace', 'suspended', 'deletion_pending', 'deleted']) { await restore(name === 'create'); await db.query('update workspaces set access_state=$1 where id=$2', [state, workspaceId]); await denied(name) }
      await restore(name === 'create'); await blocked(operations[name], `update auth_members set role='client' where id='${actor}'`, '%select * from public.lock_workspace_actor%')
      await restore(name === 'create'); await db.query("update workspaces set access_state='trial',trial_ends_at=clock_timestamp()+interval '1 second' where id=$1", [workspaceId])
    ...
    await restore(true); const beforeExpired = await snapshot()
    await assert.rejects(createTeamsOAuthSession({ ...context, authorizationId: sessionId, refreshToken: newToken, scopes: [], authorizationExpiresAt: new Date(0) }), /expir/)
    assert.deepEqual(await snapshot(), beforeExpired)
    await restore(true); const expiredAt = new Date(Date.now() + 1_000)
    await blocker.query('begin'); await blocker.query('lock table audit_events in share mode')
    const beforeDeadline = await snapshot(), deadlinePending = Promise.allSettled([createTeamsOAuthSession({ ...context, authorizationId: sessionId, refreshToken: newToken, scopes: [], authorizationExpiresAt: expiredAt })])
    ...
    await restore(true); const created = await create(); const afterCreate = await snapshot(); assert.equal(afterCreate.sessions.length, 1); assert.equal(afterCreate.sessions[0].id, created.id); assert.equal(decryptSecret(afterCreate.sessions[0].encrypted_refresh_token), newToken)
    await assert.rejects(create(), /changé/)
    const older = await beginTeamsOAuthSession(context), newer = await beginTeamsOAuthSession(context)
    const currentSnapshot = await snapshot(), callCount = providerCalls
    await assert.rejects(accessTeamsOAuthSession({ ...context, sessionId: newer.id }), /changé/)
    await assert.rejects(createTeamsOAuthSession({ ...context, authorizationId: older.id, refreshToken: newToken, scopes: [], authorizationExpiresAt: older.expiresAt }), /expir/)
    assert.deepEqual(await snapshot(), currentSnapshot); assert.equal(providerCalls, callCount)
    await createTeamsOAuthSession({ ...context, authorizationId: newer.id, refreshToken: newToken, scopes: [], authorizationExpiresAt: newer.expiresAt })
    await db.query("update auth_members set role='client' where id=$1", [actor])
    const beforeBegin = await snapshot(); await assert.rejects(beginTeamsOAuthSession(context), /non autorisée/); assert.deepEqual(await snapshot(), beforeBegin)
    await restore(); await blocked(() => beginTeamsOAuthSession(context), `update auth_members set role='client' where id='${actor}'`, '%select * from public.lock_workspace_actor%')
    ...
    await blocked(() => beginTeamsOAuthSession(context), 'lock table notification_oauth_sessions in share mode', 'delete from%notification_oauth_sessions%', 'trial')
    console.log(JSON.stringify({ ok: true, verified: ['three_current_actor_lifecycle_denials', 'three_membership_waits', 'three_trial_wait_rollbacks', 'four_session_expiry_waits', 'refresh_reauthorization_for_rotated_and_unchanged_token', 'deleted_and_concurrently_rotated_session_denied', 'current_quota_after_downgrade', 'foreign_user_and_expired_session_denied', 'expiry_and_plan_change_during_refresh', 'authorization_expiry_before_and_after_audit_wait', 'encrypted_tokens_and_single_concurrent_completion', 'pending_session_never_refreshes', 'older_callback_cannot_replace_newer_authorization', 'consumed_callback_denied', 'begin_actor_wait_and_trial_rollback'], simulatedProviderCalls: providerCalls, realProviderCalls: 0 }))
  } finally {
